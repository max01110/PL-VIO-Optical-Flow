...
void img_callback(const sensor_msgs::ImageConstPtr &img_msg)
{
    ...
    // ROS_INFO("IN LINE FEATURE CALLBACk (1)");
    ...
    // ROS_INFO("IN LINE FEATURE CALLBACk (2)");

    cv_bridge::CvImageConstPtr ptr = cv_bridge::toCvCopy(img_msg, sensor_msgs::image_encodings::MONO8);
    // ROS_INFO("IN LINE FEATURE CALLBACk (2.1)");
    cv::Mat show_img = ptr->image;
    // ROS_INFO("IN LINE FEATURE CALLBACk (2.2)");

    //    cv::imshow("lineimg",show_img);
    //    cv::waitKey(1);
    TicToc t_r;
    frame_cnt++;
    // ROS_INFO("IN LINE FEATURE CALLBACk (2.3)");
    trackerData.readImage(ptr->image.rowRange(0, ROW)); // rowRange(i,j) 取图像的i～j行
    // ROS_INFO("IN LINE FEATURE CALLBACk (3)");


    for (unsigned int i = 0;; i++)
    {        
        bool completed = false;
        if (!STEREO_TRACK)                   //monocular or non-binocular
            completed |= trackerData.updateID(i);   // LineFeatureTracker
        if (!completed)
            break;
    }
    ...
    if (PUB_THIS_FRAME)
    {
        ...
        for (int i = 0; i < NUM_OF_CAM; i++)
        {
            if (i != 1 || !STEREO_TRACK) // 单目
            {
                ...
                ROS_INFO("============== IDS NUMBER: %i", ids.size());

                // ROS_INFO("line ids: %i", trackerData.curframe_->lineID[0]);
                ...
                for (unsigned int j = 0; j < ids.size(); j++)
                {
                    ...
                    // ROS_INFO("un_lines[j].EndPt.y: %f", un_lines[j].EndPt.y);
;
        ...
        // ROS_INFO("publish ---- u_of_endpoint.size(): %i", u_of_endpoint.values.size());
        // ROS_INFO("publish ---- u_of_endpoint.values[0]: %f", u_of_endpoint.values[0]);
...
int main(int argc, char **argv)
{
    ...
    // ROS_INFO("IN LINE FEATURE TRACKER");
